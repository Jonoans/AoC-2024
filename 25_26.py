...
button_pat = re.compile(r"(X|Y)\+(\d+)")
coord_pat = re.compile(r"(X|Y)=(\d+)")

machines = machines.strip().splitlines()
machines = [machines[i:i+3] for i in range(0, len(machines), 4)]
machines = [
        [tuple(int(y[1]) for y in button_pat.findall(x)) for x in m[0:2]] +
        [tuple(int(y[1]) for y in coord_pat.findall(x)) for x in m[2:]] for m in machines
]
...
for machine in machines:
    button_a = machine[0]
    button_b = machine[1]
    win_position = tuple(map(lambda x: x + 10000000000000, machine[2]))
    visited = set()

    b_coef = button_b[0] * button_b[1]
    a_x_coef = button_a[0] * button_b[1]
    a_y_coef = button_a[1] * button_b[0]
    a_x_less_a_y = a_x_coef - a_y_coef
    x_less_y = win_position[0] * button_b[1] - win_position[1] * button_b[0]
    a_val = x_less_y / a_x_less_a_y
    if (a_val % 1) != 0:
        continue

    b_val = (win_position[0] * button_b[1] - a_val * a_x_coef) / b_coef
    print(b_val)
    if (b_val % 1) != 0:
        continue

    print(a_val, b_val)
    answer += int(a_val * 3 + b_val * 1)
print(answer)
